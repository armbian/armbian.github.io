...
Used by .github/workflows/maintenance-update-readme.yml, which checks out each
target repo, runs this script, and opens a pull request with the result. Also
runnable locally:

    ANTHROPIC_API_KEY=... python3 scripts/generate_readme.py \\
        --repo-dir ../configng --repo-name armbian/configng
...
SYSTEM = (
    ...
    "commands, badges, URLs, file or directory paths, or install steps you cannot see.\n"
    "- When describing the repository layout or referencing any directory/file, use only "
    "paths that appear in the provided file tree/layout. Do not guess or pattern-complete "
    "sibling names (e.g. do not add config/desktop/ just because config/cli/ and "
    "config/distributions/ exist). If unsure a path exists, leave it out.\n"
    ...
    "- Attribute a language by the file's own type, not by code embedded inside it: "
    ...
    "  <br><br>\n"
    "</h2>\n"
    "- After the logo header, add an H1 title, then open the body with a "
    "\"## Purpose of This Repository\" section giving a one-to-two sentence description "
    "of what the repository is for.\n\n"
